    ...
    <div style={{ position: "relative", width: "100vw", height: "100vh", overflow: "hidden", fontFamily: "Georgia, serif" }}>
      ...
      <div style={{ position: "relative", zIndex: 10, display: "flex", flexDirection: "column", height: "100vh" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "22px 32px 18px", borderBottom: "1px solid rgba(90,62,43,0.12)" }}>
          <button onClick={() => navigate("/")} style={{ background: "none", border: "none", cursor: "pointer", color: "#a08060", fontFamily: "Georgia, serif", display: "flex", alignItems: "center", gap: 6, fontSize: 13 }}>
            <i className="ri-arrow-left-line" style={{ fontSize: 16 }} /> back
          </button>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <i className="ri-delete-bin-2-line" style={{ fontSize: 22, color: "#7a5a3e" }} />
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "28px 40px" }}>
          {deletedWraps.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 14 }}>
              <i className="ri-delete-bin-2-line" style={{ fontSize: 52, color: "#c4a07a", opacity: 0.25 }} />
                ...
                return (
                  <div key={entry.year} style={{ background: "rgba(255,252,248,0.88)", borderRadius: 18, overflow: "hidden", boxShadow: "0 4px 20px rgba(90,62,43,0.08)" }}>
                    <div style={{ display: "flex", alignItems: "center", padding: "16px 22px", gap: 12, borderBottom: isOpen ? "1px solid rgba(180,140,100,0.15)" : "none", cursor: "pointer" }}
                      ...
                      <i className="ri-archive-line" style={{ fontSize: 22, color: "#c4a07a", opacity: 0.6 }} />
                      <div style={{ flex: 1 }}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); restoreWrap(entry.year); navigate("/treasure-chest") }}
                        style={{ background: "rgba(212,184,150,0.35)", border: "none", borderRadius: 14, padding: "6px 14px", fontSize: 11, color: "#5a3e2b", cursor: "pointer", fontFamily: "Georgia, serif", display: "flex", alignItems: "center", gap: 5 }}>
                        <i className="ri-arrow-go-back-line" /> restore
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); setConfirmPerm(entry.year) }}
                        style={{ background: "rgba(180,80,60,0.1)", border: "none", borderRadius: 14, padding: "6px 14px", fontSize: 11, color: "#a04030", cursor: "pointer", fontFamily: "Georgia, serif", display: "flex", alignItems: "center", gap: 5 }}>
                        <i className="ri-delete-bin-6-line" /> delete forever
                      </button>
                      <i className={`ri-arrow-down-s-line`} style={{ fontSize: 16, color: "#c4a07a", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }} />
                    </div>
                        ...
                        {folderNames.map((name) => {
                          ...
                          return (
                            <div key={name} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "10px 14px", borderRadius: 12, background: empty ? "transparent" : `${col}44` }}>
                              ...
                              <div style={{ flex: 1 }}>
                                <p style={{ fontSize: 12, fontWeight: 600, color: "#3d2b1a", textTransform: "capitalize", marginBottom: 2, display: "flex", alignItems: "center", gap: 5 }}>
                                  {name.replace("_", " ")} <i className={FOLDER_ICONS[name]} style={{ fontSize: 12, color: "#a08060" }} />
                                </p>
                                {empty ? (
      ...
      {confirmPerm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.35)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#fffaf5", borderRadius: 20, padding: "32px 36px", width: 340, boxShadow: "0 8px 40px rgba(0,0,0,0.15)", textAlign: "center", fontFamily: "Georgia, serif" }}>
            <i className="ri-delete-bin-6-line" style={{ fontSize: 44, color: "#a04030" }} />
